        ...
        <thead>
            <tr>
                ...
                <th @click="$wire.sortField('rewards.name')" class="p-2 whitespace-nowrap border border-spacing-1 cursor-pointer">
                    <x-sort :$sortDirection :$sortBy :field="'rewards.name'" /> Customer
                </th>
                ...
                <th @click="$wire.sortField('rewards.name')" class="p-2 whitespace-nowrap border border-spacing-1 cursor-pointer">
                    <x-sort :$sortDirection :$sortBy :field="'rewards.name'" /> Name
                </th>
            </tr>
            <tr>
            ...
            @foreach ($data as $reward)
                <tr>
                    ...
                    <td class="p-2 whitespace-nowrap border border-spacing-1">
                        <x-button @click="$dispatch('dispatch-reward-table-edit', { id: '{{ $reward->id }}' })" type="button">
                            Edit
                        </x-button>
                        <x-danger-button @click="$dispatch('dispatch-reward-table-delete', { id: '{{ $reward->id }}', name: '{{ $reward->name }}' })">
                            Delete
                        </x-danger-button>
                    </td>
                    <td class="p-2 whitespace-nowrap border border-spacing-1">{{ $reward->customer_name }}</td>
                    <td class="p-2 whitespace-nowrap border border-spacing-1">{{ $reward->month }}</td>
                    <td class="p-2 whitespace-nowrap border border-spacing-1">{{ $reward->year }}</td>
                    <td class="p-2 whitespace-nowrap border border-spacing-1">{{ $reward->reward_name }}</td>
                </tr>
            @endforeach
            @endisset
        </tbody>
